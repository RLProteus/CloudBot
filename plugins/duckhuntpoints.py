...
@hook.command("bang", "ba", autohelp=False)
def bang(nick, chan, message, db, conn, notice):
    ...
    if not game_status[network][chan]['game_on']:
        ...
        if game_status[network][chan]['no_duck_kick'] == 1:
            ...
            out = "Stop shooting at nothing! You missed the duck by {:.3f} seconds!".format(shoot - deploy)
        ...
        duck = "point" if score == 1 else "points"
        message("{} you shot a duck worth {} points in {} seconds! You have {} duck kill {} in {}.".format(nick, points, timer, score, duck, chan))
        set_ducktime(chan, conn)

@hook.command("befriend", "be", autohelp=False)
def befriend(nick, chan, message, db, conn, notice):
    ...
    if not game_status[network][chan]['game_on']:
        ...
        if game_status[network][chan]['no_duck_kick'] == 1:
            ...
            deploy = game_status[network][chan]['duck_time']
            shoot = game_status[network][chan]['shoot_time']
            out = "Imaginary friends aren't allowed here... You missed the duck by {:.3f} seconds!".format(shoot - deploy)
            conn.cmd("KICK", chan, nick, out)
            return "Get out of here! There is no duck. You missed the duck by {:.3f} seconds!".format(shoot - deploy)
        return "You tried befriending a non-existent duck, that's fucking creepy. You missed the duck by {:.3f} seconds!".format(shoot - deploy)
    else:
        game_status[network][chan]['shoot_time'] = time()
        deploy = game_status[network][chan]['duck_time']
        shoot = game_status[network][chan]['shoot_time']
        ...
        duck = "point" if score == 1 else "points"
        timer = "{:.3f}".format(shoot - deploy)
        message("{} you befriended a duck worth {} points in {} seconds! You have {} duck friend {} in {}.".format(nick, points, timer, score, duck, chan))
        set_ducktime(chan,conn)
...
@hook.command("duckpull", autohelp=False, permissions=["botcontrol"])
def duck_pull(chan, message, bot, notice):
    """- This command starts a duckhunt in your channel, to stop the hunt use .stophunt"""

    notice("Attempting to deploy a duck")
    global game_status
    for network in game_status:
        if network not in bot.connections:
            continue
